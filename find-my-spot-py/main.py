    ...
    while True:
        ...
        for i, frame in enumerate(frames):
            if ret[i] == True:
                ...
                imgThreshold[i] = cv2.adaptiveThreshold(imgBlur[i], 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                    cv2.THRESH_BINARY_INV, 25, 16)
                imgMedian[i] = cv2.medianBlur(imgThreshold[i], 5)
                kernel = np.ones((3, 3), np.uint8)
                imgDilate[i] = cv2.dilate(imgMedian[i], kernel, iterations=1)
                
                freeSpaceCounter[i] = checkParkingSpace(rescaledFrames[i], imgDilate[i], posList[i], height[i], width[i])
